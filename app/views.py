...
@views.route('/professorCourse.html/<id>', methods=['GET', 'POST'])
@login_required
def prof_course(id):
    professor = Professor.query.filter_by(prof_id=current_user.get_id()).first()
    course = Course.query.filter_by(course_id=id).first()
    if course is None:
        return render_template('/professorCourseNotFound.html')
    professor_c = Book_Professor_Course.query.filter_by(course_id=id).first()
    professor_check = Professor.query.filter_by(prof_id=professor_c.prof_id).first()
    if (professor_check.prof_id != professor.prof_id):
        return render_template('/professorNotCourse.html', course_name=course.name,
                               professor_name=str(professor_check.last_name) + ". "
                                              + str(professor_check.first_name), course_semester=course.semester,
                               course_year=course.year,
                               course_description=course.summary, course_image=course.image)
    books = []
    bookCourse = Book_Professor_Course.query.filter_by(course_id=course.course_id)
    for bc in bookCourse:
        book_id = bc.book_id
        books.append(Book.query.filter_by(book_id=book_id).first())
    return render_template('/professorCourse.html', course_name=course.name,
                           professor_name=str(professor.last_name) + ". "
                                          + str(professor.first_name), course_semester=course.semester,
                           course_year=course.year, books=books,
                           course_description=course.summary, course_image=course.image)

@views.route('/professorBooks', methods=['POST', 'GET'])
def professor_books():
    if request.method != "POST":
        books = []
        id = current_user.get_id()
        courses = ProfCourse.query.filter_by(prof_id=id)
        for i in range(courses.count()):
            book_id = BookCourse.query.filter_by(course_id=courses[i].course_id)
            for j in range(book_id.count()):
                book = Book.query.filter_by(book_id=book_id[i].book_id).first()
                books.append(book)
        return render_template('professorBooks.html',books=books)
    else:
        ...
        if (str(choice1) == 'None'):
            if (str(choice2) == '2000+'):
                books = Book.query.filter(Book.published_year >= 2000)
            elif (str(choice2) == '2005+'):
                books = Book.query.filter(Book.published_year >= 2005)
            elif (str(choice2) == '2010+'):
                books = Book.query.filter(Book.published_year >= 2010)
            elif (str(choice2) == '2015+'):
                books = Book.query.filter(Book.published_year >= 2015)
            print("select1 is " + str(choice1))
        else:
            print("select2 is " + str(choice2))
            if (str(choice2) == 'None'):
                if (str(choice1) == '1+'):
                    books = Book.query.filter(Book.rating >= 1.0)
                elif (str(choice1) == '2+'):
                    books = Book.query.filter(Book.rating >= 2.0)
                elif (str(choice1) == '3+'):
                    books = Book.query.filter(Book.rating >= 3.0)
                elif (str(choice1) == '4+'):
                    books = Book.query.filter(Book.rating >= 4.0)
            else:
                rate1 = float(str(choice1)[0] + ".0")
                rate2 = int(str(choice2)[0:4])
                books = Book.query.filter(Book.rating >= rate1,Book.published_year>rate2)
        if books is not None:
            for i in range(books.count()):
                bookz.append(books[i])
        return render_template('professorBookProfile.html',books=bookz)
...
@views.route('/professorAddRemoveBooks.html')
@login_required
def professor_add_remove_books():
    return render_template('/professorAddRemoveBooks.html')

"""@views.route('/professorCourse.html/<id>/<c_name>')
@login_required
def search_course(id,c_name):
    ...
    if course is None:
        return render_template('/professorCourseNotFound.html')
    professor_c = Book_Professor_Course.query.filter_by(course_id=course.course_id).first()
    professor_check = Professor.query.filter_by(prof_id=professor_c.prof_id).first()
    if (professor_check.prof_id != professor.prof_id):
...
def professor_home():
    ...
    courses = []
    professor_c = Book_Professor_Course.query.filter_by(prof_id=user)
    print("prof_c is " + str(professor_c))
    for i in range(professor_c.count()):
        course_id = professor_c[i].course_id
        print("course id is " + course_id)
        course = Course.query.filter_by(course_id=course_id).first()
        if course is not None:
           courses.append(course)
    #if request.method != 'POST':
    return render_template('professorHome.html',pid=user,courses=courses)
    #else:
       # name = request.form.get('search')
       # return redirect(url_for('views.search_course',id=user,c_name=name))

"""@views.route('/professorBookProfile.html')
@login_required
def search_rate_year():
    choice1 = request.form.get('sectionChoice1')
    choice2 = request.form.get('sectionChoice2')
    books = None
    bookz = []
    if (str(choice1) == 'None'):
        if (str(choice2) == '2000+'):
        ...
        if (str(choice2) == 'None'):
            ...
            rate1 = float(str(choice1)[0] + ".0")
            rate2 = int(str(choice2)[0:4])
            books = Book.query.filter(Book.rating >= rate1 and Book.published_year >= rate2)
    if books is not None:
        for i in range(books.count()):
            bookz.append(books[i])
    return render_template('professorBookProfile.html')"""
